...
const riemannZeta = function riemannZeta(n: number): number {
  ...
  return (
    (Math.pow(2, n) * Math.pow(Math.PI, n - 1) * polygamma(n - 1, 1)) / fact(n)
  );
};

function besselEquationSolver(
  n: number,
  xMax: number,
  stepSize: number
): number[] {
...
//timoshenkoEquation
function timoshenkoEquationSolver(
  E: number,
  I: number,
  beta1: number,
  beta2: number,
  ...
  for (let x = 0; x <= xMax; x += stepSize) {
    ...
    const deltaWTriplePrime =
      stepSize *
      ((F(x) -
        2 * beta1 * wDoublePrime ** 2 -
        2 * beta2 * wDoublePrime * wTriplePrime -
        beta3 * wDoublePrime * wDoublePrime * wPrime -
        beta4 * wTriplePrime) /
        (E * I));
...
function sirhModelSolver(
  ...
  for (let i = 1; i < numSteps; i++) {
    ...
    const dI =
      (beta * (S[i - 1] * I[i - 1])) / N - gamma * I[i - 1] - delta * I[i - 1];
...
class BeltConveyor {
  ...
  calculateWearRate(): number {
    ...
    const wearRate =
      0.001 * this.materialHardness * this.beltSpeed * this.contactPressure;
...
function classical_RK4(
  ...
  for (let i = 0; i < numSteps; i++) {
    ...
    const k2 = f(
      t0 + (i + 0.5) * dt,
      y.map((yi, j) => yi + 0.5 * dt * k1[j])
    );
    const k3 = f(
      t0 + (i + 0.5) * dt,
      y.map((yi, j) => yi + 0.5 * dt * k2[j])
    );
    const k4 = f(
      t0 + (i + 1) * dt,
      y.map((yi, j) => yi + dt * k3[j])
    );

    y = y.map(
      (yi, j) => yi + (dt / 6) * (k1[j] + 2 * k2[j] + 2 * k3[j] + k4[j])
    );
  }
...
//ロジスティック差分式
function logisticDifferenceEquation(
  r: number,
  K: number,
  P0: number,
  numSteps: number
  ...
  interpolate(x: number): number {
    ...
    for (let i = 0; i < n; i++) {
      ...
      for (let j = 0; j < n; j++) {
        if (j !== i) {
          term =
            (term * (x - this.dataPoints[j].x)) /
            (this.dataPoints[i].x - this.dataPoints[j].x);
        }
      }
...
class NaturalSplineInterpolator {
  ...
  interpolate(x: number): number {
    ...
    for (let i = 0; i < n - 1; i++) {
      ...
      if (x >= xi && x <= xi1) {
        ...
        result =
          a * this.dataPoints[i].y +
          b * this.dataPoints[i + 1].y +
          (((a ** 3 - a) * this.coefficients[i][1] +
            (b ** 3 - b) * this.coefficients[i + 1][1]) *
            h ** 2) /
            6.0 +
          (((a ** 2 - 1) * this.coefficients[i][2] +
            (b ** 2 - 1) * this.coefficients[i + 1][2]) *
            h) /
            2.0;
        break;
      }
  ...
  private calculateCoefficients(): number[][] {
    ...
    for (let i = 1; i < n - 1; i++) {
      alpha.push(
        (3 / h[i]) * (this.dataPoints[i + 1].y - this.dataPoints[i].y) -
          (3 / h[i - 1]) * (this.dataPoints[i].y - this.dataPoints[i - 1].y)
      );
    }
    ...
    for (let i = 1; i < n - 1; i++) {
      l[i] =
        2 * (this.dataPoints[i + 1].x - this.dataPoints[i - 1].x) -
        h[i - 1] * mu[i - 1];
      mu[i] = h[i] / l[i];
    ...
    for (let j = n - 2; j >= 0; j--) {
      c[j] = z[j] - mu[j] * c[j + 1];
      b[j] =
        (this.dataPoints[j + 1].y - this.dataPoints[j].y) / h[j] -
        (h[j] * (c[j + 1] + 2 * c[j])) / 3;
      d[j] = (c[j + 1] - c[j]) / (3 * h[j]);
    }
...
const rkf45 = function rkf45(
  ...
  while (t < tEnd) {
    ...
    // 次のステップの予測値
    const yNext =
      y + (25 * k1) / 216 + (1408 * k3) / 2565 + (2197 * k4) / 4104 - k5 / 5;
